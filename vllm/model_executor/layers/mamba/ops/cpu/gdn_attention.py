...
def cpu_gdn_attention_core(
    ...
    if num_decodes > 0:
        ...
        k_dim = layer.key_dim // layer.tp_size
        v_dim = layer.value_dim // layer.tp_size
        query, key, value = torch.split(decode_mixed_qkv, [q_dim, k_dim, v_dim], dim=-1)
        query = query.view(num_decode_tokens, -1, layer.head_k_dim)
        key = key.view(num_decode_tokens, -1, layer.head_k_dim)
        value = value.view(num_decode_tokens, -1, layer.head_v_dim)

        if query.shape[1] != value.shape[1]:
            repeat_factor = value.shape[1] // query.shape[1]
            query = query.repeat_interleave(repeat_factor, dim=1)
            key = key.repeat_interleave(repeat_factor, dim=1)

        query = l2norm(query, dim=-1, eps=1e-6).float().mul_(
            layer.head_k_dim**-0.5
        )
        key = l2norm(key, dim=-1, eps=1e-6).float()
        value = value.float()
