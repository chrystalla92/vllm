...
def apply_top_p_topk_candidates(
    ...
    The production CPU path allows host synchronization already.  For the
    common top-p-only case this avoids a full ascending vocab sort; instead it
    sorts only a high-probability candidate prefix and scatters those logits
    back into the original tensor after filling the rest with -inf.
    """
    vocab_size = logits.shape[1]
    if k is None:
        # Qwen production sampling commonly uses top_p=0.8/0.9 without top_k.
        # 4096 candidates is enough headroom for the nucleus while cutting the
        # sort working set by an order of magnitude for large vocabularies.  If
        # the distribution is unusually flat, fall back to full sort so the
        # nucleus is exact.
    ...
    # Keep the first token that reaches the threshold, and always keep at least one.
